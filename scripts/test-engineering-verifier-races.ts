...
for (const kind of ["run", "principal"]) {
  ...
  guard.child.stdin!.write(`begin;set local idle_in_transaction_session_timeout='15s';set local application_name=${q(guardName)};
    select id from engineering_private.native_runtime_admissions where id=${q(f.runtime)} for update;\n`);
  let completion;
  try {
    ...
    completion = call({ ...f, actor: principal }, `public.api_complete_native_verification(${q(run)},${q(JSON.stringify(verified.context))})`, name, "engineering_native_verifier")
      .then(value => ({ value }), error => ({ error }));
    await waitFor(`select exists(select 1 from pg_stat_activity where application_name=${q(name)} and wait_event='transactionid')`);
  ...
  assert.match(outcome?.error?.stderr ?? "", /PT409|42501/, `${kind} expiry after the finalization lock wait must prevent commit`);
...
console.log(JSON.stringify({ outcome: "passed", postWaitRunAndPrincipalExpiry: true, simulatedNativeAdmissions: true, productionChanged: false }));

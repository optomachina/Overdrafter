...
const runner=path.resolve('scripts/test-engineering-native-ownership.mjs');
const refusal='Pass exactly one disposable local OVD-498/501 container';

describe('native ownership test runner admission',()=>{
  it.each([[],['postgres://example.test/database'],['production-db'],['supabase_db_ovd501-']])(
  ...
  it('refuses extra output paths without modifying the destination',async ()=>{
    const directory=await mkdtemp(path.join(tmpdir(),'ovd501-cli-'));
    const destination=path.join(directory,'preserve.json');
    try {
      await writeFile(destination,'preserved fixture');
      const result=await exec(process.execPath,[runner,'supabase_db_ovd501-fixture',destination],

...
function hasMatchingProviderIdentity(manifests, { key, domain, canonicalUrl }) {
...
async function assertPathAbsent(targetPath, message) {
  try {
...
async function publishProviderFiles({ integrationsDir, providerDir, key, files }) {
  ...
  try {
    ...
    await assertPathAbsent(
      providerDir,
      `provider path already exists and will not be overwritten: ${providerDir}`,
    );
...
export async function scaffoldProvider({ rootDir, url, dryRun = false }) {
  ...
  if (hasMatchingProviderIdentity(manifests, { key, domain, canonicalUrl })) {
    return {
      key,
      domain,
      dryRun,
  ...
  await publishProviderFiles({
    integrationsDir: path.dirname(providerDir),
    providerDir,
    key,
    files,
  });

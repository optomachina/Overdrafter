...
const runner = path.resolve("scripts/test-engineering-queue.mjs");

describe("engineering queue test runner admission", () => {
  it("refuses an output-path argument before accessing Docker or overwriting a file", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "ovd497-cli-"));
    ...
    try {
      ...
      const result = await exec(process.execPath,
        [runner, "supabase_db_ovd497-engineering-queue", destination],
        { env: { ...process.env, PATH: directory } }).catch((error) => error);
      expect(result.code).not.toBe(0);
      expect(result.stderr).toContain("Pass exactly one disposable local OVD-496/497 container");

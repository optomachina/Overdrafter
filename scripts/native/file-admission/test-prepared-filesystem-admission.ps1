...
New-Item -ItemType Directory -Path (Join-Path $input 'parts') -Force -ErrorAction Stop | Out-Null
New-Item -ItemType Directory -Path $output -ErrorAction Stop | Out-Null
$required = @('synthetic-assembly.SLDASM', 'parts\baseline-5mm.SLDPRT', 'parts\candidate-8mm.SLDPRT')
foreach ($name in $required) {
    $path = Join-Path $input $name
    [IO.File]::WriteAllText($path, ('synthetic ' + $name), (New-Object Text.UTF8Encoding($false)))
}
$originalHashes = @($required | ForEach-Object { (Get-FileHash -LiteralPath (Join-Path $input $_) -Algorithm SHA256).Hash })
...
function Expect-Rejection([string]$Name, [scriptblock]$Action) {
    ...
    catch { Record $Name $true $_.Exception.GetType().Name }
}
...
$guard = $null
try {
    $guard = [PreparedFilesystemAdmission]::Begin($input, $output, $attempt.ToString('D'))
    ...
    foreach ($name in $required) { [IO.File]::Copy((Join-Path $input $name), (Join-Path $candidate $name), $false) }
    ...
    Expect-Rejection 'input_write_denied' {
        $file = New-Object IO.FileStream((Join-Path $input $required[0]), [IO.FileMode]::Open,
            [IO.FileAccess]::Write, [IO.FileShare]::ReadWrite)
        $file.Dispose()
    }
    Expect-Rejection 'input_file_replacement_denied' { [IO.File]::Move((Join-Path $input $required[0]), (Join-Path $input 'moved.SLDASM')) }
    Expect-Rejection 'attempt_replacement_denied' { [IO.Directory]::Move($folder, ($folder + '-moved')) }
    Expect-Rejection 'candidate_directory_replacement_denied' { [IO.Directory]::Move($candidate, ($candidate + '-moved')) }
    $guard.Recheck()
...
$mklink = Start-Process -FilePath 'cmd.exe' -ArgumentList @('/c', 'mklink', '/J', ('"' + $junctionInput + '"'), ('"' + $input + '"')) -Wait -PassThru -NoNewWindow
if ($mklink.ExitCode -ne 0) { throw 'Could not create synthetic junction.' }
Expect-Rejection 'junction_denied' {
...
$mklink = Start-Process -FilePath 'cmd.exe' -ArgumentList @('/c', 'mklink', '/D', ('"' + $symlinkInput + '"'), ('"' + $input + '"')) -Wait -PassThru -NoNewWindow
...
foreach ($name in $required) {
    if ($name -eq $required[0]) {
        ...
        $mklink = Start-Process -FilePath 'cmd.exe' -ArgumentList @('/c', 'mklink', '/H', ('"' + $link + '"'), ('"' + (Join-Path $input $name) + '"')) -Wait -PassThru -NoNewWindow
        if ($mklink.ExitCode -ne 0) { throw 'Could not create synthetic hard link.' }
    } else { [IO.File]::Copy((Join-Path $input $name), (Join-Path $candidate $name), $false) }
}
$guard = $null
try {
    $guard = [PreparedFilesystemAdmission]::Begin($input, $output, $attempt2.ToString('D'))
    $guard.BindAttemptDirectory($folder)
    $guard.BindCandidateDirectories($candidate)
    Expect-Rejection 'descendant_hardlink_denied' { $guard.BindCandidateFiles() }
} finally { if ($null -ne $guard) { $guard.Dispose() } }
...
try {
    ...
    Expect-Rejection 'substituted_drive_denied' {
        $alias = [PreparedFilesystemAdmission]::Begin($mappedInput, $output, ([Guid]::NewGuid().ToString('D')))
        $alias.Dispose()
    }
} finally { & subst.exe $drive /D | Out-Null }

$afterHashes = @($required | ForEach-Object { (Get-FileHash -LiteralPath (Join-Path $input $_) -Algorithm SHA256).Hash })
Record 'source_files_unchanged' (($originalHashes -join ',') -ceq ($afterHashes -join ',')) 'Exact SHA-256 readback'
$passed = @($cases | Where-Object { -not $_.passed }).Count -eq 0 -and $cases.Count -eq 12

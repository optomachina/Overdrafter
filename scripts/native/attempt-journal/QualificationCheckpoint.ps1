...
function New-PreparedQualificationCheckpoint([string]$Boundary,$Binding,$Journal,$Owner) {
    if ($Boundary -cnotin @('native_launch_intent','native_identity','outputs_saved','native_exit')) { throw 'Unsupported qualification boundary.' }
    Assert-JournalBinding $Binding
    ...
    if ($summary.failureCode -or $Journal.records.Count -eq 0) { throw 'Qualification requires an acknowledged non-failed boundary.' }

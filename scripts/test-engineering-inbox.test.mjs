...
const runner = path.resolve("scripts/test-engineering-inbox.mjs");

describe("engineering inbox test runner admission", () => {
  it("refuses an output-path argument before accessing Docker or overwriting a file", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "ovd496-cli-"));
    ...
    try {
      ...
      const result = await exec(process.execPath,
        [runner, "supabase_db_ovd496-engineering-inbox", destination],
        { env: { ...process.env, PATH: directory } }).catch((error) => error);
      expect(result.code).not.toBe(0);
      expect(result.stderr).toContain("Exactly one disposable local container argument is required");
  ...
  it("rejects a hosted connection string before accessing Docker", async () => {
    ...
    expect(result.code).not.toBe(0);
    expect(result.stderr).toContain("No remote database connections are supported");
    expect(result.stderr).not.toContain("spawn docker");
  });
});

...
select is(
  (
    ...
    where package_fingerprint = private.part_package_fingerprint(
      repeat('c', 64), repeat('d', 64)
    )
  ),
  2,
  'private analytics can observe the cross-organization exact match'
);
...
select ok(
  ...
  'authenticated callers cannot inspect global fingerprint observations'
);
...
select lives_ok(
  $$
    select public.api_finalize_existing_part_intake(
      current_setting('test.source_part_version_id')::uuid,
      'Bracket placement'
    )
  $$,
  'finalizing an exact match creates a lightweight placement atomically'
);
...
select is(
  (
    select count(distinct part.part_version_id)::integer
    from public.parts part
    where part.organization_id = '82000000-0000-4000-8000-000000000003'
      and part.part_version_id = (
        select source_part.part_version_id
        from public.parts source_part
        where source_part.id = '82000000-0000-4000-8000-000000000016'
      )
  ),
  1,
  'source and reused placements point to one immutable version'
);
...
select is(
  ...
  2,
  'exact reuse preserves independent project/job placements'
);
...
select is(
  (
    public.api_reuse_trusted_part_version_artifacts(
      (
        select target.id from public.parts target
        where target.organization_id = '82000000-0000-4000-8000-000000000003'
          and target.id <> '82000000-0000-4000-8000-000000000016'
...
select is(
  (
    ...
    join public.parts target on target.id = extraction.part_id
    where target.organization_id = '82000000-0000-4000-8000-000000000003'
      and target.id <> '82000000-0000-4000-8000-000000000016'
  ),
  1,

...
select ok(to_regprocedure('public.insert_published_quote_option(uuid,public.client_option_kind,uuid,integer,numeric,numeric,text,uuid)') is not null,
...
select ok(not has_function_privilege('anon', 'public.insert_published_quote_option(uuid,public.client_option_kind,uuid,integer,numeric,numeric,text,uuid)', 'EXECUTE'),
  'anon cannot execute the helper, including inherited PUBLIC grants');
select ok(not has_function_privilege('authenticated', 'public.insert_published_quote_option(uuid,public.client_option_kind,uuid,integer,numeric,numeric,text,uuid)', 'EXECUTE'),
...
insert into auth.users (id, aud, role, email, email_confirmed_at, raw_app_meta_data) values
  ('00000000-0000-4000-8000-000000004191', 'authenticated', 'authenticated', 'publication-internal@example.test', now(), '{"provider":"email"}'::jsonb),
  ('00000000-0000-4000-8000-000000004192', 'authenticated', 'authenticated', 'publication-client@example.test', now(), '{"provider":"email"}'::jsonb);
insert into public.organizations (id, name, slug) values
  ('00000000-0000-4000-8000-000000004193', 'Publication security fixture', 'publication-security-fixture');
...
insert into public.jobs (id, organization_id, created_by, title, status, active_pricing_policy_id) values
  ('00000000-0000-4000-8000-000000004195', '00000000-0000-4000-8000-000000004193', '00000000-0000-4000-8000-000000004191', 'Publication fixture', 'internal_review', '00000000-0000-4000-8000-000000004194');
...
insert into public.quote_runs (id, job_id, organization_id, initiated_by, status) values
  ('00000000-0000-4000-8000-000000004197', '00000000-0000-4000-8000-000000004195', '00000000-0000-4000-8000-000000004193', '00000000-0000-4000-8000-000000004191', 'completed');
insert into public.vendor_quote_results (id, quote_run_id, part_id, organization_id, vendor, requested_quantity, status, total_price_usd) values
  ('00000000-0000-4000-8000-000000004198', '00000000-0000-4000-8000-000000004197', '00000000-0000-4000-8000-000000004196', '00000000-0000-4000-8000-000000004193', 'xometry', 1, 'instant_quote_received', 100);
...
select throws_ok($$select public.insert_published_quote_option('00000000-0000-4000-8000-000000004199', 'lowest_cost'::public.client_option_kind, '00000000-0000-4000-8000-000000004198', 1, 0, 0.01, 'attacker')$$,
  '42501', null, 'anon direct invocation is denied before the helper body');
reset role;

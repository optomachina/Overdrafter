...
create function pg_temp.set_provider_notification_request_identity(p_user_id uuid)
returns void
language plpgsql
...
begin
  ...
  perform pg_catalog.set_config('request.jwt.claim.role', 'authenticated', true); -- NOSONAR: stable role fixture
end;
$$;

create temporary table provider_notification_test_context (
  admin_user_id uuid not null,
  member_user_id uuid not null
) on commit drop;

insert into provider_notification_test_context values (
  '00000000-0000-4000-8000-000000004861',
  '00000000-0000-4000-8000-000000004862'
);

grant select on provider_notification_test_context to authenticated;

insert into auth.users (id, aud, role, email, email_confirmed_at)
values
  (
    (select admin_user_id from provider_notification_test_context),
    'authenticated', -- NOSONAR: deterministic authenticated-role fixture
    'authenticated',
    'provider-notification-admin@example.com',
    pg_catalog.timezone('utc', pg_catalog.now())
  ),
  (
    (select member_user_id from provider_notification_test_context),
    'authenticated',
    'authenticated',
    'provider-notification-member@example.com',
    pg_catalog.timezone('utc', pg_catalog.now())
  );

insert into private.platform_admin_emails (email)
values ('provider-notification-admin@example.com');
...
select has_function(
  'public', -- NOSONAR: canonical public-schema assertion fixture
...
delete from private.quote_provider_admission_policies
where provider in ('fastdms', 'devzmanufacturing'); -- NOSONAR: transaction-local fixture reset
alter table private.quote_provider_admission_policies
...
values (
  ...
  'evidence_required',
  false,
  'provider-notification-evidence.v1',
  'initial_seed'
);
...
values (
  'devzmanufacturing',
  'disabled', -- NOSONAR: deterministic disabled-state fixture
  false,
  'provider-notification-disabled.v1',
  'initial_seed'
);
...
select ok(
  (
    select notification_type = 'provider.integration_added'
      ...
      and policy_revision = 'provider-notification-disabled.v1'
      and admission_state = 'disabled'
...
update private.quote_provider_admission_policies
set policy_revision = 'provider-notification-disabled.v2',
    change_reason = 'policy_updated'
...
select throws_ok(
  $$update private.platform_admin_notifications set policy_revision = 'provider-notification-mutated.v1'$$,
  'P0001',
...
select throws_ok(
  ...
  'P0001',
  'Platform admin notifications are append-only.', -- NOSONAR: exact trigger error assertion
...
set local role authenticated;
select pg_temp.set_provider_notification_request_identity(
  (select member_user_id from provider_notification_test_context)
);
...
select throws_ok(
  ...
  '42501',
  null,
  'service_role cannot read the private notification source directly'
);
...
select throws_ok(
  ...
  '42501',
  null,
  'service_role cannot invoke the user-facing notification reader'
);

-- OVD-160: Preserve the manufacturing_quote line-item bridge in queued vendor work.
--
-- quote_requests already links client-safe request lifecycle to service_request_line_items,
-- but run_vendor_quote payloads only carried quoteRequestId/quoteRunId. Adding the
-- authoritative line-item id to queued vendor payloads keeps request intent traceable
-- through worker task logs and retry/failure audit payloads without changing the
-- vendor_quote_results contract.
--
-- Rollback path: restore the previous api_request_quote definition and stop writing
-- serviceRequestLineItemId into work_queue payloads. Existing quote_requests and
-- service_request_line_items links remain intact.

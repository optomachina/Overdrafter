-- OVD-372: reassert two final quote-path contracts after the governed
-- production-first migration-history reconciliation.
--
-- The production ledger already records later migrations that hardened these
-- functions. Replaying older, genuinely pending migrations can otherwise
-- overwrite those later definitions without causing a migration error. Keep
-- this repair narrow and fail closed if the expected function shapes drift.
...
declare
  ...
  v_repaired_fragment constant text :=
    '''quoteRunId'', v_quote_run_id,' || chr(10)
...
begin
  ...
  if pg_catalog.strpos(v_definition, v_repaired_fragment) = 0
    or pg_catalog.strpos(v_definition, v_legacy_fragment) > 0 then
    raise exception 'Unable to restore service-request lineage in vendor tasks.';
  end if;
...
as $$
  ...
  selected_offers as (
    ...
    left join public.vendor_quote_offers offer
      on offer.id = job_row.selected_vendor_quote_offer_id
  ),
            ...
            coalesce(
              (
                select jsonb_agg(
                  ...
                  order by offer.sort_rank, offer.total_price_usd, offer.id
                )
...
begin
  ...
  if pg_catalog.strpos(v_definition, 'manufacturing_quote_line_items as (') = 0
    or pg_catalog.strpos(
      v_definition,
      'to_jsonb(offer) - ''invalidated_by'' - ''invalidation_reason'''
    ) = 0

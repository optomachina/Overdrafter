-- Restore the vendor-scope schema and API that the client quote flow expects.
--
-- The original 20260408193000 migration was skipped in the hosted project,
-- while later entitlement/rollout migrations moved automatic fan-out into
-- private.request_automatic_quote_impl(uuid, boolean). This forward migration
-- recreates the missing contract and patches the current implementation rather
-- than replaying the stale api_request_quote patch from the original migration.
--
-- Rollback:
-- 1. Restore private.request_automatic_quote_impl(uuid, boolean) to the one-
--    argument public.get_enabled_client_quote_vendors call.
-- 2. Drop the three vendor-preference RPCs and the three-argument resolver.
-- 3. Drop job_vendor_preferences and project_vendor_preferences.
...
drop policy if exists "project_vendor_preferences_select"
...
drop policy if exists "job_vendor_preferences_select"
...
drop trigger if exists touch_project_vendor_preferences_updated_at
  on public.project_vendor_preferences;
create trigger touch_project_vendor_preferences_updated_at
before update on public.project_vendor_preferences
for each row execute function public.touch_updated_at();

drop trigger if exists touch_job_vendor_preferences_updated_at
  on public.job_vendor_preferences;
create trigger touch_job_vendor_preferences_updated_at
before update on public.job_vendor_preferences
for each row execute function public.touch_updated_at();

revoke all on public.project_vendor_preferences from public, anon, authenticated;
revoke all on public.job_vendor_preferences from public, anon, authenticated;
...
as $$
  select coalesce(
    pg_catalog.array_agg(distinct item.vendor order by item.vendor),
    array[]::public.vendor_name[]
  )
  from pg_catalog.unnest(
    coalesce(p_vendors, array[]::public.vendor_name[])
  ) as item(vendor);
$$;

revoke all on function public.normalize_vendor_name_array(public.vendor_name[])
  from public, anon;
grant execute on function public.normalize_vendor_name_array(public.vendor_name[])
  to authenticated;
...
as $$
  select pg_catalog.jsonb_build_object(
    'includedVendors',
    pg_catalog.to_jsonb(
      coalesce(p_included_vendors, array[]::public.vendor_name[])
    ),
    'excludedVendors',
    pg_catalog.to_jsonb(
      coalesce(p_excluded_vendors, array[]::public.vendor_name[])
    ),
    'updatedAt',
    p_updated_at
  );
...
revoke all on function public.build_vendor_preferences_json(
  public.vendor_name[],
  public.vendor_name[],
  timestamptz
) from public, anon;
grant execute on function public.build_vendor_preferences_json(
  public.vendor_name[],
  public.vendor_name[],
  timestamptz
) to authenticated;
...
begin
  if p_project_id is not null then
    ...
    into v_project_included, v_project_excluded
  ...
  if p_job_id is not null then
    select
      coalesce(pref.included_vendors, array[]::public.vendor_name[]),
      coalesce(pref.excluded_vendors, array[]::public.vendor_name[])
    into v_job_included, v_job_excluded
  ...
  select coalesce(
    pg_catalog.array_agg(vendor_row.vendor order by vendor_row.vendor),
    array[]::public.vendor_name[]
  )
  into v_enabled_vendors
  from pg_catalog.unnest(v_baseline_vendors) as vendor_row(vendor)
  where case
    when vendor_row.vendor = any(v_job_included) then true
    when vendor_row.vendor = any(v_job_excluded) then false
    else
      (
        pg_catalog.cardinality(v_project_included) = 0
        or vendor_row.vendor = any(v_project_included)
      )
...
revoke all on function public.get_enabled_client_quote_vendors(uuid, uuid, uuid)
...
begin
  ...
  if not public.user_can_access_job(v_job.id) then
    raise exception
      'You do not have permission to inspect vendor preferences for job %.',
      p_job_id;
  end if;
  ...
  if v_job.project_id is not null then
    ...
    from public.project_vendor_preferences pref
    where pref.project_id = v_job.project_id;
  end if;

  select pref.*
  into v_job_preferences
  from public.job_vendor_preferences pref
  where pref.job_id = v_job.id;
  ...
  return pg_catalog.jsonb_build_object(
    ...
    'availableVendors', pg_catalog.to_jsonb(v_available_vendors),
    'projectVendorPreferences', public.build_vendor_preferences_json(
      coalesce(
        v_project_preferences.included_vendors,
        v_empty_vendor_array
      ),
      coalesce(
        v_project_preferences.excluded_vendors,
        v_empty_vendor_array
      ),
    ...
    'jobVendorPreferences', public.build_vendor_preferences_json(
      coalesce(
        v_job_preferences.included_vendors,
        v_empty_vendor_array
      ),
      coalesce(
        v_job_preferences.excluded_vendors,
        v_empty_vendor_array
      ),
...
revoke all on function public.api_get_job_vendor_preferences(uuid)
  from public, anon;
grant execute on function public.api_get_job_vendor_preferences(uuid)
  to authenticated;
...
declare
  ...
  v_included public.vendor_name[] := public.normalize_vendor_name_array(
    p_included_vendors
  );
  v_excluded public.vendor_name[] := public.normalize_vendor_name_array(
    p_excluded_vendors
  );
...
begin
  ...
  if not public.user_can_edit_project(v_project.id) then
    raise exception
      'You do not have permission to edit vendor preferences for project %.',
      p_project_id;
  end if;
  ...
  select coalesce(
    pg_catalog.array_agg(distinct vendor_row.vendor order by vendor_row.vendor),
    array[]::public.vendor_name[]
  )
  into v_disallowed_vendors
  from (
    select vendor from pg_catalog.unnest(v_included) as included(vendor)
    union all
    select vendor from pg_catalog.unnest(v_excluded) as excluded(vendor)
  ) vendor_row
  where vendor_row.vendor <> all(v_allowed_vendors);

  if pg_catalog.cardinality(v_disallowed_vendors) > 0 then
    raise exception
      'Vendor preferences include unsupported vendors for project %: %.',
      p_project_id,
      pg_catalog.array_to_string(v_disallowed_vendors, ', ');
  end if;
  ...
  into v_excluded
  from pg_catalog.unnest(v_excluded) as vendor_row(vendor)
  where vendor_row.vendor <> all(v_included);

  if
    pg_catalog.cardinality(v_included) = 0
  ...
  then
    ...
    values (p_project_id, v_included, v_excluded)
  ...
  return pg_catalog.jsonb_build_object('projectId', p_project_id)
    || public.build_vendor_preferences_json(
      v_included,
      v_excluded,
      v_updated_at
    );
...
revoke all on function public.api_set_project_vendor_preferences(
  uuid,
  public.vendor_name[],
  public.vendor_name[]
) from public, anon;
grant execute on function public.api_set_project_vendor_preferences(
  uuid,
  public.vendor_name[],
  public.vendor_name[]
) to authenticated;
...
begin
  ...
  if not public.user_can_edit_job(v_job.id) then
    raise exception
      'You do not have permission to edit vendor preferences for job %.',
      p_job_id;
  end if;
  ...
  then
    ...
    values (p_job_id, v_included, v_excluded)
    on conflict (job_id)
    do update set
      included_vendors = excluded.included_vendors,
      excluded_vendors = excluded.excluded_vendors
  ...
  return pg_catalog.jsonb_build_object('jobId', p_job_id)
    || public.build_vendor_preferences_json(
      v_included,
      v_excluded,
      v_updated_at
...
revoke all on function public.api_set_job_vendor_preferences(
  uuid,
  public.vendor_name[],
  public.vendor_name[]
) from public, anon;
grant execute on function public.api_set_job_vendor_preferences(
  uuid,
  public.vendor_name[],
  public.vendor_name[]
) to authenticated;

-- Patch the current entitlement-gated automatic implementation in place.
-- This preserves the Pro and rollout checks in public.api_request_quote and
-- every later guard already present in the private implementation.
do $$
declare
  v_definition text;
...
begin
  if position(
    ...
    in pg_catalog.pg_get_functiondef(
      'public.api_request_quote(uuid, boolean)'::pg_catalog.regprocedure
    )
  ) = 0 then
    raise exception
      'public.api_request_quote(uuid, boolean) does not delegate to the guarded automatic implementation.';
  end if;
  ...
  if v_definition is null then
    raise exception
      'Function private.request_automatic_quote_impl(uuid, boolean) not found.';
  end if;
  ...
  if v_updated_definition = v_definition then
    raise exception
      'Unable to patch private.request_automatic_quote_impl(uuid, boolean); expected vendor resolver call was not found.';
  end if;

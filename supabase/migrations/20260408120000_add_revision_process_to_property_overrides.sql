-- Extend seed_project_part_property_defaults to include revision and process.
...
as $$
declare
  v_defaults jsonb := coalesce(p_existing_defaults, '{}'::jsonb);
begin
  -- Delegate to the base seeding logic from the prior migration, then extend.
  -- We call the prior version's body inline here by re-implementing the full
  -- function with the new fields appended after the existing ones.
      ...
      to_jsonb(
        coalesce(
          ...
          nullif(trim(coalesce(
            p_extraction ->> 'quoteDescription',
            p_extraction ->> 'description',
            ''
          )), ''),
          null
        )
    ...
    v_defaults := v_defaults || jsonb_build_object(
      ...
      to_jsonb(
        coalesce(
          ...
          nullif(trim(coalesce(
            p_extraction ->> 'partNumber',
            p_extraction ->> 'part_number',
            ''
          )), ''),
          null
        )
  ...
  if not (v_defaults ? 'material') then
    v_defaults := v_defaults || jsonb_build_object(
      ...
      to_jsonb(
        coalesce(
          nullif(trim(coalesce(p_requirement.spec_snapshot ->> 'material', '')), ''),
...
begin
  ...
  loop
    if v_field in ('description', 'partNumber', 'material', 'finish', 'tightestToleranceInch', 'threads', 'revision', 'process') then
...
-- Also update api_update_client_part_request to pass revision and process into
-- build_project_part_property_snapshot so they're written to spec_snapshot
-- via the property snapshot (in addition to the existing direct writes).
-- This is a create or replace of the full function to thread the new args.

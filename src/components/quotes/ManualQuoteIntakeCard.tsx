...
function buildOfferPayloads(offers: OfferDraft[]): ManualQuoteOfferInput[] {
  return offers.map((offer, index) => {
    ...
    if (validityDurationDays !== null && (!Number.isInteger(validityDurationDays) || validityDurationDays <= 0)) {
      throw new Error(`Offer lane ${index + 1} has an invalid validity duration.`);
    }
    if (offer.validUntilIso && validityDurationDays !== null) {
      throw new Error(`Offer lane ${index + 1} must use either a valid-until date or a duration, not both.`);
    }
                ...
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(event) => updateOffer(offer.id, { validityDurationDays: event.target.value })}
                      placeholder="30"
                    />
                  </div>
                  <div className="space-y-2 sm:col-span-2">
                    <Label>Original validity terms</Label>
                    <Input
                      className="border-border bg-muted"

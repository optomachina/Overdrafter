...
function CollapsedSidebarRail({
  sidebarRailActions = [],
  onOpen,
}: Pick<ChatWorkspaceLayoutProps, "sidebarRailActions"> & {
  onOpen: () => void;
}) {
  return (
    <div
      className="chatgpt-shell group flex h-full cursor-e-resize flex-col items-center gap-3 bg-[#171717] px-2 py-3 text-white"
      onClick={onOpen}
    >
...
export function ChatWorkspaceLayout({
  ...
  return (
    <TooltipProvider delayDuration={SIDEBAR_TOOLTIP_DELAY_MS}>
      <div className="chatgpt-shell min-h-screen bg-[#212121] text-white">
          ...
          <div className="relative flex min-h-screen flex-1 flex-col">
            ...
            <header className="flex items-center justify-between gap-3 px-4 py-3 md:px-6">
              <Sheet open={mobileSidebarOpen} onOpenChange={setMobileSidebarOpen}>
                ...
                <SheetContent
                  side="left"
                  className="chatgpt-shell w-[260px] border-r border-white/[0.08] bg-[#171717] p-0 text-white sm:max-w-[260px] [&>button]:hidden"
                >
                  <SidebarScaffold
                    sidebarContent={sidebarContent}
                    sidebarFooter={sidebarFooter}

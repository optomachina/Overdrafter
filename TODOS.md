...
**PR sequence:**
1. **PR-A** — ✅ DONE (2026-05-14). Font loading + typography scale wiring in `tailwind.config.ts`, `index.html`, and `index.css`. Dropped Inter; loads IBM Plex Sans + Space Mono + Lab Mono (free analogs per DESIGN.md §Loading). Added `fontFamily` (`sans`/`display`/`mono`), the `fontSize` scale (`hero`→`micro`), the additive bone palette CSS vars, the Tailwind `paper-*` color namespace, and global tabular-nums. No layout changes. Lint clean, 931 tests pass.

...
- cover the atomic Xometry permit transaction with production-realistic database tests for enrollment/notice state, explicit effective provider configuration, every 1.0 envelope field (including declared inch/millimetre units), each of the three affirmations independently, stale fingerprints, replay/conflict, immutable evidence, exact lane/task binding, and zero-write legacy client request wrappers; before deployment, OVD-368 must also prove internal/service-created provider tasks cannot launch without a matching permit
